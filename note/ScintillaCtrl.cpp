...
#define AFXASSUME(cond)			do { bool __afx_condVal=!!(cond); ATLASSERT(__afx_condVal); _Analysis_assume_(__afx_condVal); } while(0)
#else
...
CScintillaCtrl::CScintillaCtrl() noexcept : m_bCallDirect(TRUE),
                                            m_DirectFunction(0),
                                            m_DirectPointer(0),
											m_SelForeColor(RGB(0, 0, 0)), 
											m_SelBackColor(RGB(94, 94, 94))
{
}

HWND CScintillaCtrl::Create(DWORD dwExStyle, LPCWSTR lpClassName, LPCWSTR lpWindowName, DWORD dwStyle, CRect rect, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam)
{
